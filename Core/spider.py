        ...
        for index, row in enumerate(self.urls):
            ...
            if oldUrlObj.netloc != '':
                ...
                if (code == 404):
                    ...
                    if (str(self.host+r.headers['Location']) != self.host+newUrlOjb.path):
                        failed += 1
                        if self.debug:
                            print("301 Redirect Mismatch found: Targeted Url: " + urlSent + " Returned Url: " + str(self.host+r.headers['Location']) + " Expected: " + self.host+newUrlOjb.path)
